    ...
    if(ctl->numDrums != 0) {
        ...
        *pos = ALIGN16(*pos);
        ...
        for (int i = 0; i < ctl->numDrums; i++){
            ...
            if(used == 0){
                int current_size = 0; // changed variable name from 'size' to 'current_size'.
                unsigned char* addr = ctlData+((uintptr_t)d->env_addr)+16;
                envData[envCount].orig = (uintptr_t)(d->env_addr);
                envData[envCount].addr = parse_envelope(addr, pos, &current_size);
                envData[envCount].size = current_size;
                envCount++;
            }
